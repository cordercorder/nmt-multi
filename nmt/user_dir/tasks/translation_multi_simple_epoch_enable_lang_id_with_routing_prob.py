...
# use different MultilingualDatasetManager to enable lang id in sample
from ..data.multilingual_data_manager import MultilingualDatasetManager
...
@register_task("translation_multi_simple_epoch_enable_lang_id_with_routing_prob")
class TranslationMultiSimpleEpochEnableLangIdWithRoutingProbTask(LegacyFairseqTask):
    """
    ...
    def add_args(parser):
        ...
        parser.add_argument('--max-routing-epoch', default=None, type=int)
    ...
    def get_batch_iterator(
        ...
        max_routing_epoch = getattr(self.args, "max_routing_epoch", self.args.max_epoch)
        self.prob = user_utils.get_routing_prob(epoch, max_routing_epoch, len(self.langs), f"cuda:{shard_id}")
    ...
    def train_step(
        ...
        if self.prob is not None:
            tgt_lang_id = sample.get("tgt_lang_id", None)
            tgt_lang_id = tgt_lang_id.squeeze(1) - 1

            # B x seq_len
            prob = self.prob.index_select(dim=0, index=tgt_lang_id)

            # compatible
            sample["net_input"]["tgt_lang_id"] = torch.multinomial(prob, 1) + 1
        else:
