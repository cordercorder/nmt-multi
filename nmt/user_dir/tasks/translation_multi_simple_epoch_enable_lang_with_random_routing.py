...
@register_task("translation_multi_simple_epoch_enable_lang_with_random_routing")
class TranslationMultiSimpleEpochEnableLangIdWithRandomRoutingTask(LegacyFairseqTask):
    """
    ...
    def __init__(self, args, langs, dicts, training):
        ...
        self.num_languages = len(self.langs)
    ...
    def inference_step(
        ...
        with torch.no_grad():
            sample["net_input"]["tgt_lang_id"] = self.get_random_tgt_id(sample)
            _, tgt_langtok_spec = self.args.langtoks["main"]
    ...
    def get_random_tgt_id(self, sample):
        _id = sample["id"]
        bsz = _id.size(0)
        tgt_lang_id = torch.randint(0, self.num_languages, (bsz, 1), device=_id.device) + 1
        return tgt_lang_id
