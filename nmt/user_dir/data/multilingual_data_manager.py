...
class MultilingualDatasetManager(object):
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            ...
            default=False,
            action="store_true",
            help="whether to include source language IDs in samples",
        )
        parser.add_argument(
            "--enable-tgt-lang-ids",
            default=False,
            action="store_true",
            help="whether to include target language IDs in samples",
        )
        parser.add_argument(
    ...
    def _shared_collater(self):
        return not (self.args.extra_data and "mono_dae" in self.args.extra_data) and (
            not (self.args.lang_tok_replacing_bos_eos or self.args.enable_src_lang_ids or self.args.enable_tgt_lang_ids)
        )
    ...
    def load_a_dataset(
        ...
        **extra_kwargs,
        ...
        enable_src_lang_ids = self.args.enable_src_lang_ids
        enable_tgt_lang_ids = self.args.enable_tgt_lang_ids
        ...
        langpair_ds = self.load_langpair_dataset(
            ...
            if enable_src_lang_ids and lang_dictionary is not None
            else None,
            tgt_lang_id=_lang_id(lang_dictionary, tgt)
            if enable_tgt_lang_ids and lang_dictionary is not None
            else None,
