...
translation_output_dir=${project_dir}/data/ted/many-many.token_tgt/translation_valid
...
for checkpoint in ${checkpoint_array[@]}; do
    ...
    for lang_pair in ${lang_pairs//,/ }; do
        ...
        sacremoses -l ${tgt_lang:0:2} detokenize < ${parallel_trans_dir}/sys.valid.${src_lang}-${tgt_lang}.${tgt_lang} > ${parallel_trans_dir}/sys.valid.detok.${src_lang}-${tgt_lang}.${tgt_lang}

        score=$(sacrebleu -w 6 ${options} ${spm_corpus_dir}/${lang_pair}/valid.spm_decode.remove_long_sentence.detok.${src_lang}-${tgt_lang}.${tgt_lang} < ${parallel_trans_dir}/sys.valid.detok.${src_lang}-${tgt_lang}.${tgt_lang})
