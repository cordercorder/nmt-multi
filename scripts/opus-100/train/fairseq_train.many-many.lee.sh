...
save_dir=${project_dir}/data/opus-100-corpus/many-many.lee/checkpoint
tensorboard_logdir=${project_dir}/data/opus-100-corpus/many-many.lee/tensorboard_logdir
...
python -m torch.distributed.launch --nproc_per_node=${n_process} \
    ...
    --langtoks "{\"extra\": (None, \"tgt\")}" \
    --arch transformer_lee \
    --layernorm-embedding \
    --remove-decoder-langtok-when-training \
    --add-lang-embedding-to-decoder-self-attention \
    --add-lang-embedding-to-decoder-cross-attention \
