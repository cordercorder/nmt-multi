...
translation_output_dir=${project_dir}/data/opus-100-corpus/many-many.token_tgt/translation_valid
...
for checkpoint in ${checkpoint_array[@]}; do
    ...
    for lang_pair in ${lang_pairs//,/ }; do
        ...
        score=$(sacrebleu -w 6 ${spm_corpus_dir}/${lang_pair}/valid.spm_decode.remove_lang_sentence.${src_lang}-${tgt_lang}.${tgt_lang} < ${parallel_trans_dir}/sys.valid.${src_lang}-${tgt_lang}.${tgt_lang})
